...
export const atualizarProduto = async (req, res) => {
    ...
    try {
        ...
        const sql = `
            UPDATE produto 
            SET nome = ?, descricao = ?, preco = ?, imagem_url = ?, fk_id_categoria = ? 
            WHERE id_produto = ?
        `;
        ...
        const [result] = await db.execute(sql, [
            ...
            precoFormatado, 
            imagem_url, 
            categoria, // Este valor deve ser o ID vindo do Select do Frontend
            id_produto 
        ]);
...
export const inativarProduto = async (req, res) => {
    ...
    try {
        ...
        res.status(500).json({ erro: "Erro ao processar a inativação." });
    }
};
...
export const reativarProduto = async (req, res) => {
    ...
    try {
        ...
        res.status(500).json({ erro: "Erro ao processar a reativação." });
    }
};